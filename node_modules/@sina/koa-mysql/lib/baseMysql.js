...
class Mysql {
     ...
     */
    _formatSql() {
        let _sql = mysql.format(this.sql, this.sqlData);
        if (this.debug) {
            console.log('<------------ SQL ------------>');
            console.log(_sql);
            console.log('<------------ SQL ------------>');
...
Mysql.prototype.query = function*(sql, master_or_slave) {
    ...
    if (this.sql.startsWith('INSERT') || this.sql.startsWith('insert')) {
        return this.rs.insertId;
    }else if (this.sql.startsWith('UPDATE') || this.sql.startsWith('update')) {
        return this.rs.changedRows;
    }else if (this.sql.startsWith('DELETE') || this.sql.startsWith('delete')) {
        return this.rs.affectedRows;
    }else {
        return this.rs;
    }
